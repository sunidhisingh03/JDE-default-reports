...
foreach ($subscription in $allsubscription) {
    ...
    foreach ($vm in $vms) {
        ...
        $startTime = (Get-Date $currentTime -Hour 0 -Minute 0 -Second 0).AddDays(-1)
        $endTime = (Get-Date $currentTime -Hour 0 -Minute 0 -Second 0).AddSeconds(-1)
...
# Export to CSV
$csvPath = "Azure_Daily_Backup_Jobs.csv"
