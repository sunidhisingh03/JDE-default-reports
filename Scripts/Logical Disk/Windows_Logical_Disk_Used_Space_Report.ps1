...
Function UsedSpaceCSVforWindows {
$Script:uswindata1 = @()
...
| summarize Average_uswin = avg(100 - CounterValue) by Computer, _ResourceId, InstanceName
...
| project Computer, SubscriptionId, Target_ResourceGroup, Average_uswin,diskname
"@
...
$dataRow = [pscustomobject]@{
                       ...
                       "CounterValue" = [math]::Round($queryResult.Average_uswin,2)
                       
                 }
              
         $Script:uswindata1 +=$dataRow
        }
    $Script:uswindata1 | Export-Csv -Path "Logical Disk Used Space for Windows Output.csv" -Force -NoTypeInformation -Encoding UTF8
      }
     }
    }

Function Top20UsedSpaceforWindows{
    try{
        $data123 = @()
        $dataRow = @()
        $Path = "./Logical Disk Used Space for Windows Output.csv"
         $Computer = @()
         $Subscription = @()
         $CounterValue = @()
         $instanceName = @()
      ...
      Function Bottom20UsedSpaceforWindows{
        try{
            $data1234 = @()
            $dataRow = @()
            $Path = "./Logical Disk Used Space for Windows Output.csv"
...
Write-Output "Fetching Used Space CSV for Windows Data"
UsedSpaceCSVforWindows

Write-Output "Fetching Top 20 Free Space CSV for Windows Data"
Top20UsedSpaceforWindows

Write-Output "Fetching Bottom 20 Free Space CSV for Windows Data"
Bottom20UsedSpaceforWindows
$htmlReport1 = $null
$script:htmlReport1 = " 
    ...
    <body> 
                ...
                <h2>Top 20 Windows - Disk Utilization Report</h2> 
                ...
                <h2>Bottom 20 Windows - Disk Utilization Report</h2> 
...
$htmlReport1 | Out-File "Logical Disk Used Space for Windows HTML.html" -Force

$fileName = "Logical Disk Used Space for Windows Output.csv"
...
$params = @{
    ...
    Body = @{
        ...
        Content = @{
            Subject = 'JDE(IT) | Azure | Windows Logical Disk Used Space'
